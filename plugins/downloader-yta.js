...
let handler = async (m, { conn, text, command }) => {
  if (!text) {
    return m.reply(`• *Contoh:* .${command} https://www.youtube.com/xxxxxxx`);
  }
  ...
  try {
    ...
    m.reply(`🎉 *Berhasil!* Musik dari *${searchResponse.title}* udah di-download! 🎶`);
  } catch (error) {
    console.error(error);
    m.reply('❌ Terjadi kesalahan saat memproses permintaanmu. Coba lagi nanti.');
  }
};
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (mp4ConvertResponse.data.status !== 'ok') {
                throw new Error('Gagal mengonversi video ke MP4.');
            }
